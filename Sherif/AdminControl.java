public class AdminControl {
    ...
    {
        //return true if the product added successfully and false if it already exists in the system
    	return DataBase.addProduct(product);
    }
    
    public static boolean addBrand(Brand brand)
    {
    	return DataBase.addBrand(brand);
    }
    ...
    public static void provideVoucherCard()
    {
        ...
        while(sz>0)
        {
        	...
        	int j=4;
          while (j>0) {
          for(int i=0,k=step*j;i<4;i++,k+=sz-11)srial+=alpha[k%26];
          j--;
          if (j>0)srial+='-';
          }
